#!/usr/bin/env python3
"""HSV mask generator for offloading UI work."""
...
class HSVMaskNode(Node):
    def __init__(self):
        ...
        if np is None or cv2 is None:
            self.get_logger().error('OpenCV/NumPy required for hsv_mask_node')
        ...
        self.sub = self.create_subscription(Image, img_topic, self._on_image, qos)
        self.pub = self.create_publisher(Image, mask_topic, qos)
        self.get_logger().info(f"HSV mask node: {img_topic} -> {mask_topic}")

    def _on_image(self, msg: Image):
        try:
            bgr = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        except Exception as exc:
            self.get_logger().throttle(2000, f"cv_bridge failed: {exc}")
            return
