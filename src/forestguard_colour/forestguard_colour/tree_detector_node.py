...
class TreeDetector(Node):
    def __init__(self):
        ...
        self.declare_parameter('roi_ymin', 0.25)     # ignore bottom 25% (grass band)
        self.declare_parameter('min_area_px', 1200)  # contour area gate
        self.declare_parameter('aspect_min', 1.2)    # height/width gate
        ...
        self.get_logger().info(
            ...
            f"roi_ymin={self.roi_ymin} min_area={self.min_area} aspect_min={self.aspect_min}"
        )
    ...
    def _count_and_mark(self, mask, color_id, header):
        n = 0
        markers = []
        ...
        for c in contours:
            ...
            m.ns = 'trees'; m.id = self.marker_seq; self.marker_seq += 1
    ...
    def image_cb(self, msg: Image):
        ...
        # ROI: ignore bottom band (mostly grass)
        ...
        # count + markers
        num_g, m_g = self._count_and_mark(mask_g, 0, msg.header)
        num_r, m_r = self._count_and_mark(mask_r, 1, msg.header)
        counts = Int32MultiArray(); counts.data = [num_g, num_r]
        self.pub_counts.publish(counts)

        ma = MarkerArray(); ma.markers = m_g + m_r
        self.pub_markers.publish(ma)

        # debug image (overlay back into full frame)
        paint = np.zeros_like(roi)
        paint[mask_g > 0] = [0,255,0]
        paint[mask_r > 0] = [0,0,255]
        debug = frame.copy()
        debug[y0:, :] = cv2.addWeighted(roi, 0.6, paint, 0.4, 0)
