/**
* @file amcl_localization.cpp
* @brief Implementation of AMCL localization management node
* @details This file contains the implementation of the AmclLocalization class,
* which manages the initialization and fallback localization behavior for a robot
* using Adaptive Monte Carlo Localization (AMCL) in ROS2.
...
/**
* @brief Constructor initializes AMCL localization management node
...
*/
AmclLocalization::AmclLocalization()
: Node("amcl_localization_node"), localized_(false), initial_pose_sent_(false)
{
  // Declare and read parameters
  this->declare_parameter("initial_x", 0.0);
  this->declare_parameter("initial_y", 0.0);
  this->declare_parameter("initial_yaw", 0.0);
  this->declare_parameter("cov_threshold", 0.2);
  ...
  amcl_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
    "/amcl_pose", 10, std::bind(&AmclLocalization::amclPoseCallback, this, std::placeholders::_1));

  timer_ = this->create_wall_timer(1s, std::bind(&AmclLocalization::timerCallback, this));

  RCLCPP_INFO(this->get_logger(), "AMCL Localization Manager started.");
}
...
* the robot has sufficiently localized. If localization is stable, the robot stops moving.
*/
void AmclLocalization::amclPoseCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
  ...
  if (avg_cov < cov_threshold_) {
    localized_ = true;
    stopMotion();
    RCLCPP_INFO(this->get_logger(), "Localization converged. Avg cov: %.5f", avg_cov);
...
* @details If no initial pose has been sent, it publishes one. If localization has not yet converged,
* the node commands the robot to rotate slowly to help AMCL collect better sensor readings.
*/
void AmclLocalization::timerCallback()
{
...
*/
void AmclLocalization::publishInitialPose()
{
...
*/
void AmclLocalization::stopMotion()
{
  geometry_msgs::msg::Twist twist;
  cmd_vel_pub_->publish(twist);
}
